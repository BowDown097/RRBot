...
namespace RRBot.Preconditions
{
    ...
    public class RequireStaffAttribute : PreconditionAttribute
    {
        ...
        public override async Task<PreconditionResult> CheckPermissionsAsync(ICommandContext context, CommandInfo command, IServiceProvider services)
        {
            ...
            return (snap.TryGetValue("houseRole", out ulong staff1Id) && (context.Message.Author as IGuildUser).RoleIds.Contains(staff1Id))
            || (snap.TryGetValue("senateRole", out ulong staff2Id) && (context.Message.Author as IGuildUser).RoleIds.Contains(staff2Id))
                ? PreconditionResult.FromSuccess()
                : PreconditionResult.FromError($"{context.Message.Author.Mention}, you must be Staff!");
        }
    }
}
